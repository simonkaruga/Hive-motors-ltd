...
export default function FilterBar({ onFilterChange, totalCount, filteredCount, initialFilters }: FilterBarProps) {
  ...
  return (
    <div className="mb-8">
      {/* Top bar: mobile toggle + results count + sort */}
                ...
                <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-200">
                  {Object.entries(filters).map(([key, value]) => {
                    ...
                    return (
                      <span
                        key={key}
                        className="inline-flex items-center gap-1.5 bg-navy-brand/10 text-navy-brand text-xs font-medium px-3 py-1.5 rounded-full"
                      >
                        <span className="text-mid-grey text-[10px]">{label}:</span> {display}
                        <button
                          onClick={() => handleChange(key as keyof FilterState, '')}
                          aria-label={`Remove ${label} filter`}
                          className="hover:text-red-brand ml-0.5"
                        >
                          <X size={11} />
                        </button>
                      </span>
