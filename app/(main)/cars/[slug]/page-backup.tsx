...
export async function generateStaticParams() {
  try {
    ...
    console.log('Generated static params:', [...sanityParams, ...staticParams]);
...
export default async function CarDetailPage({ params }: Props) {
  ...
  // Debug logging
  console.log('Car detail page - slug:', slug);
  ...
  const car = await (async () => {
    try {
      const car = await client.fetch(carBySlugQuery, { slug });
      console.log('Sanity car fetch result:', car);
      
      if (!car) {
        console.log('No car found for slug:', slug);
        notFound();
      }
